    ...
    for cond in conditions:
        ...
        for c, cat in enumerate(categories):
            if c == 0:
                ...
                cat_name = f"{categories[c - 1] + 1}-{cat}"
                count = np.sum((val > categories[c - 1]) & (val <= cat))
                clt_obj_num_cat.loc[cat_name, f"{cond}"] = count
            else:
                cat_name_0 = f"{categories[c - 1] + 1}-{cat}"
...
def avg_results_stem(results, stems, filt=False):
    
    def avg_stem(
            df_stm_avg, results=results, filt=filt,
    ...
    df_stm_avg["obj_num"  ] = [len(df) for df in obj_stm]
    df_stm_avg["clt_num"  ] = [len(df) for df in clt_stm]
    df_stm_avg["clt_ratio"] = df_stm_avg["clt_num"] / df_stm_avg["obj_num"]
    for tag in ["obj", "clt"]:
...
def avg_results_cond(results, conditions, filt=False):
    f = "_f" if filt else ""
    df_stm_avg = results[f"results_stm_avg{f}"]
...
def process_results(
    ...
    results = {}
    filt = min_clt_obj_num > 1 or min_clt_area > 1
    ...
    # Average stem
    results["results_stm_avg"  ] = avg_results_stem(
        results, stems, filt=False)
    if filt:
        results["results_stm_avg_f"] = avg_results_stem(
            results, stems, filt=True)
    
    # Average cond
    results["results_cnd_avg"  ] = avg_results_cond(
        results, conditions, filt=False)
    if filt:
        results["results_cnd_avg_f"] = avg_results_cond(
            results, conditions, filt=True)
...
def plot_clusters(
    ...
    tags_0 = ["obj_num", "clt_num", "clt_ratio", "clt_obj_num_avg"]
    tags_1 = [
        "clt_area_avg", "clt_intensity_avg",
        "clt_circularity_avg", "clt_solidity_avg", "clt_eccentricity_avg",
        ]
    tags_2 = ["clt_obj_num_dist"]
    tags_3 = ["clt_obj_num_cat"]
    tags = tags_0 + tags_1 + tags_2 + tags_3
    ...
    for i, (ax, tag) in enumerate(zip(axes, tags)):
        for c, cond in enumerate(conditions):    
            ...
            if tag in tags_1:
                ...
                f = "_f" if "clt_results_all_f" in results[chn] else ""
                
                # Data
                avg, sem = (
                    results[chn][f"results_cnd_avg{f}"].loc[f"{cond}_avg", tag],
                    results[chn][f"results_cnd_avg{f}"].loc[f"{cond}_sem", tag],
                    )
            ...
            if tag in tags_3 and segmentation == "instance":
            ...
            if tag in tags_1:
                ...
                for ch, chn in enumerate(chns):
                    ...
                    if tag == "mix_ratio_avg":
                        ...
                        avg, sem = (
                            results[chn][f"results_cnd_avg{f}"].loc[
                                f"{cond}_avg", tag],
                            results[chn][f"results_cnd_avg{f}"].loc[
                                f"{cond}_sem", tag],
                            )
